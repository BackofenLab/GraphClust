...
my (
    ...
    $i_fas,                $i_wins,       $i_shift,
    $i_crop_unpaired_ends, $i_r,          $i_e,
    $i_c,                  $i_t,          $i_u,
    $i_M,                  $i_o,          $i_sge,
    $i_jobid,              $i_tmp,        $i_ignore_seq_header,
    $i_stacks,             $i_stdout,     $i_q,
    $i_T,                  $i_i,          $i_sample_min_length,
...
if ( $i_t !~ /^\d+$/ ) {
    ...
    foreach my $idx ( 1 .. @t_minlens ) {
        ...
        die
"Wrong -t format! First level given needs to be length 0! Example: -t 3=0,4=100,5=200\n"
          if ( $idx == 1 && $lev_len[1] != 0 );
        die
"Wrong -t format! Only level 1-5 allowed! Example: -t 3=0,4=100,5=200\n"
          if ( $lev_len[0] < 1 or $lev_len[0] > 5 );
        die
          "Wrong -t format! Length >= 0 expected! Example: -t 3=0,4=100,5=200\n"
          if ( $lev_len[1] < 0 );
    ...
    ($i_debug)
      and print STDERR
      "i_t = $i_t - change $change_shape_level -  shape level lengths#"
      . join( ":", @level_lens ) . "#"
      . join( ":", @t_minlens ) . "#\n";
}
...
my ( $headers_aref, $sequences_aref, $metainfo_aref ) =
  read_fasta_with_nonunique_headers_meta($i_fas);
...
"mv $gspanfile.no_match $i_o/$group_idx.group.gspan.no_match"
    ...
    if (@struct_meta) {
        $graph_header = getGraphHeader(
            $seq_id,              $seq_header,           \@WINDOWS,
            $i_shift,             $i_e,                  $i_c,
            $i_t,                 $i_u,                  $i_r,
            $i_M,                 $i_crop_unpaired_ends, $i_i,
            $i_sample_min_length, $i_q,                  $i_T,
            $seq_len,             1
        );
    }
    else {
        $graph_header = getGraphHeader(
            ...
            $i_t,                  $i_u,
            $i_r,                  $i_M,
            $i_crop_unpaired_ends, $i_i,
            $i_sample_min_length,  $i_q,
            $i_T,                  $seq_len,
            $i_no_structure || $no_structure_override
        );
    }
    ...
    if ( @struct_meta && !$i_no_structure && !$no_structure_override ) {
        ...
        my $seq_shrep = [ $tmp_meta->{$struct_meta}, "ENERGY", "0.00", "SHAPE",
            $struct_meta ];
        $gi =
          convertShapeWindow( [$seq_shrep], $seq, $seq_len, 1, $gi, $out,
            $graph_header, $i_annotate, $i_abstr, $i_crop_unpaired_ends,
            $i_stacks, $seq );
    ...
    foreach my $win_size (@WINDOWS_used) {
        ...
        ($i_debug)
          and print STDERR "winsize: $win_size curr_shift: $curr_shift\n";
        ...
        $rnashapesoutput_fh = call_RNAshapes(
            $seq_fasta,           $rnashapes_loc, $win_size,
            $curr_shift,          $i_e,           $i_c,
            $curr_t,              $i_u,           $i_r,
            $i_q,                 $i_T,           $i_i,
            $i_sample_min_length, $seq_len,       $i_matchShape
        );

        # read RNAshapes output from $rnashapesoutput_fh and write subgraph
        # to gspan file
        my $gi_old = $gi;
        $gi = convert_RNAshapes_output(
            $rnashapesoutput_fh, $gi,                   $i_M,
            $out,                $graph_header,         $win_size,
            $seq_len,            $curr_t,               $i_annotate,
            $i_abstr,            $i_crop_unpaired_ends, $i_stacks,
            $seq
        );

        ## no (match) shape found at all for this seq
        if ( $gi == $gi_old + 1 ) {
            $gi = convertSeqWindow( "X", 1, 1, $gi, $graph_header, $out,
                $i_annotate, $seq );
...
############################################################################
sub call_RNAshapes {
    my ( $seq_fasta, $rnashapes_location, $win_size, $shift, $e, $c, $t, $u,
        $r, $q, $T, $i, $sample_length, $seqLen, $matchShape )
      = @_;
    my $FUNCTION = "call_RNAshapes in fasta2shrep_gspan.pl";

    ($seq_fasta)
      or die("INPUT ERROR in $FUNCTION: the fasta file is compulsory!\n");
    ($rnashapes_location)
      or die( "INPUT ERROR in $FUNCTION: the RNAshapes location"
          . " is compulsory!\n" );
    ...
    $call .= "-W $shift "
      if ($i_shift);             ##### before it was just --windowIncrement

    die(
"ERROR in $FUNCTION: Give only one of the options -c or -e (RNAshapes)!\n"
      )
    ...
    $call .= "-i $i -A "
      if (
            not $q
        and $i
        and ( $win_size >= $sample_length
            and ( $win_size <= $seqLen or $seqLen >= $sample_length ) )
      );    ## -A is to omit samples and print only combined shape probs
    $call .= " < $seq_fasta";

    ($i_debug) and print STDERR "$seqLen $sample_length $win_size $call\n";

    open my $rnashapesoutput, "$call |"
      or die( "ERROR in $FUNCTION: The following call "
...
sub convert_RNAshapes_output {
    my (
        $rnashapesoutput, $curr_gi, $maxShreps, $graph_file_hdl,
        $graphHead,       $winSize, $seqLen,    $used_t,
        $annotate,        $abstr,   $cue,       $stacks,
        $orig_seq
    ) = @_;
    ...
    while ( $line = <$rnashapesoutput> ) {
        ...
        if ( $line =~ /^(\d+)\s+(\d+)$/ ) {
            ...
            if ( @{$win_shreps} > 0 ) {
                ...
                if ($i_add_seq_graph_win) {
                    ($curr_gi) =
                      convertSeqWindow( $win_seq, $win_size_real,
                        $win_start, $curr_gi, $winHead, $graph_file_hdl,
                        $annotate, $orig_seq );
                }

                $curr_gi = convertShapeWindow(
                    $win_shreps, $win_seq,  $win_size_real,
                    $win_start,  $curr_gi,  $graph_file_hdl,
                    $winHead,    $annotate, $abstr,
                    $cue,        $stacks,   $orig_seq
                );
            }
            ...
            $winHead =
              getWindowHeader( $graphHead, $winSize, $win_start, $win_end,
                $win_center, $win_globalFolding, $win_sample, $used_t );

        }
        elsif ( $line =~ /^(\S+)$/ ) {
            ## line: "CUUAUGAGUAAGGAAAAUAACGAUUCGGGGUGACGCCCGAAUCCUCACUG"
            $win_seq =
              uc($1); ## to be 101% sure that we have by default uppercase chars
            ...
            push(
                @{$win_shreps},
                [ $1, "ENERGY", $2, "SHAPEPROB", $3, "SHAPE", $4 ]
            );
            $win_shrep_count++;
            ...
            $line =~ /^([\(\)\.]+)\s+\((\S+)\)\s+\((\S+)\)\s+(\S+)\s+(\S+)$/ )
        {
            ...
            push(
                @{$win_shreps},
                [ $1, "ENERGY", $2, "PROB", $3, "SHAPEPROB", $4, "SHAPE", $5 ]
            );
            $win_shrep_count++;
    ...
    if ( @{$win_shreps} > 0 ) {
        ...
        if ($i_add_seq_graph_t) {
            my @new_win_shreps = ();
            map { push( @new_win_shreps, $_ ) if ( $_->[ $#{$_} ] ne "_" ) }
              @{$win_shreps};
            $win_shreps = \@new_win_shreps;
        }
        ...
        $curr_gi = convertShapeWindow(
            $win_shreps, $win_seq,        $win_size_real, $win_start,
...
sub getGraphHeader {
    my (
        $seq_id,       $seq_head, $wins_aref,            $h_shift,
        $h_e,          $h_c,      $h_t,                  $h_u,
        $h_r,          $h_M,      $h_crop_unpaired_ends, $h_i,
        $h_sample_len, $h_q,      $h_Tp,                 $h_seqlen,
        $h_noStr
    ) = @_;
