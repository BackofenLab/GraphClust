...
if (   not defined $fast_cluster
    || not defined $data_fasta
    || not defined $fast_cluster_sim
    || not defined $map_data )
{
    die
"Wrong arguments: candidate clusters, fasta file, similarity scores and/or map data are missing \n";
}
...
sub writeSubsetFrags {
    ...
    foreach my $key ( sort { $a <=> $b } @{$set_aref} ) {
        ...
        die
"FASTA header wrong! Fragment key ($seqprefix) (eg. 'SEQ123#45#67#+')  not found in header $head! Exit...\n\n"
          if ( $frag eq "MISS" );
        print OUT "$key " . $frag . "\n";
    }
    close(OUT);
...
sub sim2matrix {
    ...
    my $knn = @{$ids}
      ;    ## knn x knn matrix to build, sim file contains kernel similarities
    die "Sim matrix line has incorrect number of entries! Expected "
      . ( $knn * ( $knn - 1 ) / 2 )
      . " found "
      . @ent
      . " Exit...\n\n"
...
sub matrix_blacklist_overlap_frags {
    ...
    my $black_overlap =
      $_[4];    ## minimal required overlap of two frags to become blacklisted
    ...
    my $matrix_ols =
      fragment_overlap( $matrix_frags, $matrix_frags, $black_overlap, 1 );

    ## store all overlaps in hash
    my %idOls = ();
    foreach my $ol ( @{$matrix_ols} ) {
        $idOls{ $matrix_frags->[ $ol->[0] ]->{VALUE} . "#"
              . $matrix_frags->[ $ol->[1] ]->{VALUE} } = $ol->[2];
    }
    ...
    while ( my $line = <MAT> ) {
        ...
        foreach my $col ( 0 .. $#ent ) {
            ...
            if (   exists $idOls{ $ids_matrix[$row] . "#" . $ids_matrix[$col] }
                || exists $idOls{ $ids_matrix[$col] . "#" . $ids_matrix[$row] }
              )
            {
...
sub mergeFrags {
    ...
    if ( @{$del} ) {
        my @frag_list_del =
          grep { exists $to_delete{ $_->{FRAGID} } }
          sort { $a->{SEQID} cmp $b->{SEQID} } values(%frags);
        my $ols = fragment_overlap( \@frag_list_del, \@frag_list, 0.1, 1 );
    ...
    foreach my $seq ( keys %olmap ) {
          ...
          )
        {
            ...
            foreach my $ol ( @{$ols} ) {
                ...
                $fr[ $ol->[0] ]->{START} =
                  min( $fr[ $ol->[0] ]->{START}, $fr[ $ol->[1] ]->{START} );
                $fr[ $ol->[0] ]->{STOP} =
                  max( $fr[ $ol->[0] ]->{STOP}, $fr[ $ol->[1] ]->{STOP} );
                $fr[ $ol->[0] ]->{KEY} =
                    $fr[ $ol->[1] ]->{SEQID} . "#"
                  . $fr[ $ol->[0] ]->{START} . "#"
                  . $fr[ $ol->[0] ]->{STOP} . "#"
                ...
                if ( $fr[ $ol->[0] ]->{STRAND} eq "+" ) {
                    $fr[ $ol->[0] ]->{SEQ} = substr( $fr[ $ol->[0] ]->{SEQ},
                        0,
                        $fr[ $ol->[1] ]->{START} - $fr[ $ol->[0] ]->{START} );
                    $fr[ $ol->[0] ]->{SEQ} .= $fr[ $ol->[1] ]->{SEQ};
                }
                else {
                    $fr[ $ol->[0] ]->{SEQ} = substr(
                        $fr[ $ol->[0] ]->{SEQ},
                        $old0_stop - $fr[ $ol->[1] ]->{START} + 1,
                        $old0_len -
                          ( $old0_stop - $fr[ $ol->[1] ]->{START} ) - 1
                    );
                    $fr[ $ol->[0] ]->{SEQ} =
                      $fr[ $ol->[1] ]->{SEQ} . $fr[ $ol->[0] ]->{SEQ};
                }
                $fr[ $ol->[0] ]->{MERGED_STR} .= ";"
                  . $fr[ $ol->[1] ]->{FRAGID} . ":"
                  . $fr[ $ol->[1] ]->{MERGED};
    ...
    foreach my $nf (@newFrags) {
        ...
        $newHeads{ $nf->{FRAGID} } =
          $nf->{KEY} . " MERGED " . $nf->{MERGED_STR} . " " . $nf->{ORIGHEAD};
    }
