...
foreach my $file (sort(@tabFiles)) {
  ...
  $round_max = $round if ( $round > $round_max );
    ...
    if ( !exists $exist_part_used{$key} ) {
        $cm_hits = read_CM_tabfile_ext( $file, $cm_min_bitscore, $cm_max_eval,
            $cm_bitscore_sig, $key );
    ...
    ## used for do_overlap_matrix, is equal to CLASS_CLUSTER for tabfile hits only
    $cm_hitlists{"$round.$index"}->{CLASS} = 0 ;
...
foreach my $round ( 1 .. $round_max ) {
    ...
    print "\n round:$round " . "\n";
    my @keys = grep { $cm_hitlists{$_}->{ROUND} <= $round } sort {
        $cm_hitlists{$a}->{ROUND} <=> $cm_hitlists{$b}->{ROUND}
          || $cm_hitlists{$a}->{INDEX} <=> $cm_hitlists{$b}->{INDEX}
    } keys %cm_hitlists;
    ...
    write_partition( $all_hits, $clus2idx, $old2new_map_SOFT,
        "EVAL/partitions/$round.soft" );
    ...
    write_partition( $hits, $clus2idx, $old2new_map_BEST,
        "EVAL/partitions/$round.hard.best" );

    ## merged partitions
    ## overlap matrix is based on %cm_hitlists, i.e. on soft, not on hard partition of hits
    my $old2new_map_MERGED =
      do_overlap_matrix( $old2new_map_SOFT, "EVAL/overlap/$round.cm_overlap",
        $old2new_map_ORIG, $merge_overlap, $merge_cluster_ol );
    $old2new_map_MERGED = filterClusters( $hits, $old2new_map_MERGED );
    $clus2idx = get_clus2idx( \%cm_hitlists, $old2new_map_MERGED );
    write_partition( $hits, $clus2idx, $old2new_map_MERGED,
        "EVAL/partitions/$round.hard.merged" );
...
## soft part data
system(
    "cp EVAL/partitions/$round_max.soft RESULTS/partitions/final_partition.soft"
);
...
system(
"cp EVAL/partitions/$round_max.hard.merged RESULTS/partitions/final_partition.hard.merged"
);
system(
"cp EVAL/partitions/$round_max.hard.best RESULTS/partitions/final_partition.hard.best"
);
system(
"cp EVAL/overlap/$round_max.cm_overlap RESULTS/partitions/final_overlap.matrix"
);
system(
"cp EVAL/overlap/$round_max.cm_overlap.merge_map RESULTS/partitions/final_overlap.map"
);
...
sub do_overlap_matrix {
  ...
  #    sort { $cm_hitlists{$a}->{CLASS} <=> $cm_hitlists{$b}->{CLASS} } @keys;
    ...
    print CMO "MODEL CLASS "
      . join( " ", map { $cm_hitlists{$_}->{CLASS} } @keys ) . "\n";
    ...
    foreach my $i ( 0 .. $#keys ) {
        ...
        foreach my $j ( 0 .. $#keys ) {
            ...
            my $listOLs = fragment_overlap(
                $cm_hitlists{ $keys[$i] }->{HITS},
                $cm_hitlists{ $keys[$j] }->{HITS},
                $merge_ol, 0
            );
            ...
            foreach my $ol ( sort { $b->[2] <=> $a->[2] } @{$listOLs} ) {
                if (
                    !exists $uniq_ols_i{ $cm_hitlists{ $keys[$i] }->{HITS}
                          ->[ $ol->[0] ]->{KEY} } )
                {

                    $uniq_ols_j{ $cm_hitlists{ $keys[$j] }->{HITS}
                          ->[ $ol->[1] ]->{KEY} } = 1;
                    $uniq_ols_i{ $cm_hitlists{ $keys[$i] }->{HITS}
                          ->[ $ol->[0] ]->{KEY} } = 1;
                }
            }
...
#}
            ...
            $best_ol = [ ( $ol / $num ), $keys[$i], $keys[$j], $ol, $num ]
              if ( "$i" ne "$j" && $num > 0 && ( $ol / $num ) > $best_ol->[0] );

            push( @ols, $str );

        }
        $ol_map{ $best_ol->[1] . "#" . $best_ol->[2] } = $best_ol
          if ( $best_ol->[0] >= $merge_cluster_overlap );
        print CMO $keys[$i] . " "
          . $cm_hitlists{ $keys[$i] }->{CLASS} . " "
          . join( " ", @ols ) . "\n";
    ...
    my @merge_pairs =
      sort { $ol_map{$b}->[0] <=> $ol_map{$a}->[0] } keys %ol_map;
    ...
    foreach my $pair (@merge_pairs) {
        ...
        foreach my $old_child ( keys %{$merge_map} ) {
            $merge_map->{$old_child} = $parent
              if ( $merge_map->{$old_child} eq $child );
        }
    ...
    foreach my $par (@parents) {
        my @childs =
          sort { $a cmp $b }
          grep { $merge_map->{$_} eq $par } keys %{$merge_map};
...
sub filterClusters {
    ...
    foreach my $clus (@parentClusters) {
        ...
        map {
            push( @hits, $_ )
              if ( exists $mergeMap->{ $_->{NAME} }
                && $mergeMap->{ $_->{NAME} } eq $clus )
        } @{$partition};
...
#my @hits = grep { $mergeMap->{ $_->{NAME} } eq $clus } grep{ exists $mergeMap->{ $_->{NAME} } } @{$partition};
        ...
        if ( @hits >= $min_cluster_size ) {
            map {
                $mergeMap_filtered{$_} = $mergeMap->{$_}
                  if ( $mergeMap->{$_} eq $clus )
            } keys %{$mergeMap};
        }
...
sub get_clus2idx {
    ...
    @keys = sort {
        $clusters->{$a}->{ROUND} <=> $clusters->{$b}->{ROUND}
          || $clusters->{$a}->{INDEX} <=> $clusters->{$b}->{INDEX}
    } unique( values %{$merge_map} );
    my %merg2idx = ();
...
sub do_ORACLE_merge {
    ...
    my %merge_map_NEW = ();
    my @keys =
      sort { $clusters->{$a}->{ROUND} <=> $clusters->{$b}->{ROUND} }
      keys %{$merge_map};
    my %classMap = ();
    ...
    map {
        if ( $clusters->{$_}->{CLASS} ne "0" ) {
            $merge_map_NEW{$_} = $classMap{ $clusters->{$_}->{CLASS} };
        }
        else { $merge_map_NEW{$_} = $_ }
    } @keys;
...
sub write_partition {
    ...
    my @clus =
      sort { $a <=> $b } unique( map { $clus2idx->{$_} } keys %{$merge_map} );

    foreach my $idx (@clus) {

        my @hits = grep {
            exists $clus2idx->{ $_->{NAME} }
              && $clus2idx->{ $_->{NAME} } == $idx
        } @{$partition};
        ...
        foreach my $hit ( sort { $b->{BITSCORE} <=> $a->{BITSCORE} } @hits ) {
            ...
            print OUT join( " ",
                $hit->{KEY}, $hit->{BITSCORE}, $hit->{EVALUE}, $merge,
                $hit->{NAME}, $idx );
...
sub get_f_measure {
    ...
    foreach my $idx (@clus) {
        ...
        my $clus_class =
          GraphClust::evaluate_cm_hits( $class_Frags, \@hits, $min_class_OL );

        my @clusters = unique( map { $_->{NAME} } @hits );
        my @class    = unique( map { $_->{CLASS_CLUSTER} } @hits );

        print "\nWarning! Mergin of different classes! classes="
          . join( ":", @class )
          . " clus="
          . join( ":", @clusters ) . "\n"
          if ( @class > 1 );
        ...
        foreach my $hit (@hits) {
            if ( $hit->{CLASS} ne "0" && $hit->{CLASS} eq $clus_class ) {
                ...
                $fp++
                  if ( ( $hit->{CLASS} eq "0" && $evaluate_class0_as_fp )
                    || $hit->{CLASS} ne "0" );
            }
        }
        ...
        if ( $clus_class ne "0" ) {
            ...
            $tn = scalar( $class_Size->{"0"} )
              ;    ## tn is only for accuracy, could be skipped
        }
        else {
            $tp = 0;
            $fp = 0;
        ...
        my @ent = (
            $idx, "cluster",
            join( ":", @clusters[ 0 .. min( $#clusters, 10 ) ] ),
            "CLASS", $clus_class, "HITS", scalar(@hits)
        );
        ## idx 7-22
        push(
            @ent,
            (
                "F",  $f,  "ACC", $a,  "PREC", $p,  "RECALL", $r,
    ...
    my @all = (
        scalar(@res), "clusters", scalar(@res), "classes", 0, "hits", 0, "F",
        0, "ACC", 0, "PREC", 0, "RECALL", 0
    );
    ...
    ## check if we have clusters to analyze
    if (@res) {
        map { $all[$_] /= scalar(@res) }
          @cols[ 1 .. 4 ];              ## not for "hits"-column
        map { $all[$_] = sprintf( "%.3f", $all[$_] ) }
          @cols[ 1 .. 4 ];              ## not for "hits"-column
    }
...
sub write_res_summary {
    ...
    foreach my $idx ( 1 .. @fm_sort ) {
        ...
        print $OUT_SUM " CLUSTER "
          ...
          . " CENTER "
          . $res[2];
        print $OUT_SUM " HIT_NUM "
          . $res[6] . " "
          . join( " ", @res[ 15 .. 24 ] );
        print $OUT_SUM "\n";

        if ( $evaluate_class0_as_fp
            || ( $res[4] != "0" && !$evaluate_class0_as_fp ) )
        {
            $f_sum += $res[8];
            $clus_count++;
        }
        ...
        if ( $idx - 1 == $#fm_sort
            || floor( ( $res[2] =~ /^(\d+)\.\d+/ )[0] ) ne
            floor( ( $fm_sort[$idx]->[2] =~ /^(\d+)\.\d+/ )[0] ) )
        {
            ...
            $str .=
              ...
              . ( keys %classes )
              . " FMEASURE_AVG "
              . sprintf( "%.3f", $f_sum / $clus_count );

            my $cl_unique_ratio = "0.000";
            $cl_unique_ratio =
              sprintf( "%.3f", ( keys %classes ) / ( keys %{$class_Size} ) )
              if ( keys %classes );

            $str .=
              " CL_FINAL " . @centers_curr . " CL_UNIQ_REL " . $cl_unique_ratio;
            ...
            $fstep_str .=
              " FM_AVG_BETTER_05 " . sprintf( "%.3f", $tf / unique(@classes) );
...
sub makeUniqueHits {
    ...
    while ( $hits_curr_start_idx < @{$hits} ) {
        ...
        while ($hits_curr_end_idx < @{$hits} - 1
            && $curr_frag->{SEQID} eq
            $hits->[ $hits_curr_end_idx + 1 ]->{SEQID} )
        {
            $hits_curr_end_idx++;
        }
      ...
      #   print "OL:".$ols->[0]->[0]." ".$ols->[0]->[1]." ".$ols->[0]->[2]."\n";
                ...
                map {
                    push( @better, $curr_hits->[ $_->[1] ] )
                      if (
                        $curr_hits->[ $_->[1] ]->{BITSCORE} > $hit->{BITSCORE} )
                } @{$ols};
...
sub read_CM_tabfile_ext {
    ...
    open( TABRES, "$tab_file" )
      or die "$tab_file tab_result file could not be loaded!";
...
#    @cm_hit_scores = () if ( $lineLast !~ /^\#/ ); ## simple check that cmsearch finished correctly
    ...
    if ( !$use_e_values && $significance < 1 ) {
        ...
        @cm_hit_scores =
          sort { $b->{BITSCORE} <=> $a->{BITSCORE} } @cm_hit_scores;
...
# }
    ...
    @cm_hit_scores_fil =
      sort { $a->{SEQID} cmp $b->{SEQID} } @cm_hit_scores_fil;
